<?php
  ...
  $res->execute();
  $res = $res->fetchAll();
  if(isset($_GET['sText']) && $_GET['sText'] != NULL)
  {
	$sText = $_GET['sText'];
	$sql = "SELECT * FROM passwords WHERE pw_uID = {$_SESSION['uID']} AND pw_site LIKE '%$sText%'";
	$res = $dbh->prepare($sql);
	$res->execute();
	...
	if($res->rowCount() != 0)
		...
		$err = "Нямате запезана парола в такъв сайт/приложение/игра... моля опитайте с друга ключова дума";
  }
  else
	  header('Location: myPasswords.php');
?>
<!doctype html>
<html>
<head>
